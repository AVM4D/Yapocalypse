...
using namespace std;
project::project(SocialMediaApp *app,QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::project), app(app)
{
    ui->setupUi(this);
    ...
    AnimatedBackground *bg = new AnimatedBackground(app,ui->label_4);
    bg->setGeometry(ui->label_4->rect());
    bg->lower();
...
void Object::setId(const string &newId)
{
    id = newId;
}
...
{
    ifstream File("F:/Yapocalypse/resources/Users.txt");
...
{
    ifstream file("F:/Yapocalypse/resources/Friends.txt");
    if (!file.is_open())
    {
...
{
    ifstream file("F:/Yapocalypse/resources/Pages.txt");
    if (!file.is_open())
    {
...
{
    ifstream file("F:/Yapocalypse/resources/UsersPosts.txt");
    if (!file.is_open())
    {
...
{
    ifstream file("F:/Yapocalypse/resources/PagesPosts.txt");
    if (!file.is_open())
    {
...
{
    ifstream file("F:/Yapocalypse/resources/LikedPages.txt");

    if (!file.is_open())
    {
        std::cerr << "Error opening LikedPages.txt!" << std::endl;
        return;
    }
    for (int i = 0; i < noOfUsers; i++)
    {
